...
template <typename DerivedType> struct ToolchainDescriptorInterface {
  ...
  virtual DerivedType &compiler_options_set(const Collection<String> &) = 0;
  virtual const Collection<String> &compiler_options_get() const = 0;

  virtual DerivedType &archiver_executable_set(const String &) = 0;
  virtual const String &archiver_executable_get() const = 0;

  virtual DerivedType &archiver_options_set(const Collection<String> &) = 0;
  virtual const Collection<String> &archiver_options_get() const = 0;

  virtual DerivedType &linker_executable_set(const String &) = 0;
  virtual const String &linker_executable_get() const = 0;

  virtual DerivedType &linker_options_set(const Collection<String> &) = 0;
  virtual const Collection<String> &linker_options_get() const = 0;
