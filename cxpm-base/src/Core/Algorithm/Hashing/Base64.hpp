...
namespace Core::Algorithm::Hashing {
...
public:
  ...
  signed inline static char const *inverse() {
  ...
  String encode(const String &from) {
    char result[from.size() * sizeof(uint64_t)];
    char *out = result;

    const char *in = from.c_str();
    const char *tab = alphabet();

    for (auto n = from.size() / 3; n > 0; n--) {
      *out++ = tab[(in[0] & 0xfc) >> 2];
  ...
  String decode(const String &from) {
    char dest[from.size() * sizeof(uint64_t)];
    char *out = static_cast<char *>(dest);
    auto in = reinterpret_cast<unsigned char const *>(from.data());
    unsigned char c3[3], c4[4] = {0, 0, 0, 0};
    int i = 0;
    int j = 0;
    int len = from.size();

    auto const inv = inverse();
