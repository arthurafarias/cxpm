...
|---|---|
| §2.1.2–2.1.4 (parse/write round trip, escapes, pretty-printing, malformed input) | `JsonValueCodecTest` (`CXPM/Modules/Serialization/Testing/JsonValueCodecTest.hpp`) |
| §2.1.5 (`JsonInputArchiver`) | `JsonManifestTest`: "JsonInputArchiver reads an istream into a Value tree" |
| §2.2 (descriptor conversion, defaults, malformed-field rejection) | `JsonManifestTest` (`CXPM/Modules/Serialization/Testing/JsonManifestTest.hpp`) — round-trips `TargetDescriptor`, `ToolchainDescriptor`, a multi-target/multi-toolchain `ProjectDescriptor`, and asserts `JsonParseException` on type mismatches |
