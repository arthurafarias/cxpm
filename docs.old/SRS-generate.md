...
|---|---|
| §2.2 content correctness (templates embed the right name, JSON round-trips through the real descriptor parser) | `ManifestGeneratorTest` (`CXPM/Modules/Generators/Testing/ManifestGeneratorTest.hpp`) — pure unit tests, no subprocess needed, since the generator functions are plain `String`-returning functions |
