...
1. Tests SHALL be colocated with the code they cover: each `<Subject>Test.hpp` lives in a
   `Testing/` subdirectory next to the header it tests, in the same namespace with a nested
   `::Testing` sub-namespace appended — e.g. `CXPM/Core/Containers/String.hpp` (namespace
   `CXPM::Core::Containers`) ↔ `CXPM/Core/Containers/Testing/StringTest.hpp` (namespace
   `CXPM::Core::Containers::Testing`). The test-harness infrastructure itself
   (`TestGroup`/`TestContext`/`TestCase`/`TestFailure`/`Registry`/`RunAll`/`TemporaryDirectory`),
   which is not a test of any single namespaced class, stays under the top-level
   `libraries/cxpm-interface/src/CXPM/Testing/`, in `CXPM::Testing`. Every colocated test file
   `using namespace CXPM::Testing;` to bring `TestGroup`/`TestContext`/etc. into scope, since its
   own `::Testing` sub-namespace is a sibling of `CXPM::Testing`, not nested under it. (A few
   classes' declared namespace does not match their folder — e.g. `PkgConfigManager`,
   `ProjectManager` and `ToolchainManager` live directly under `CXPM/` but declare
   `namespace CXPM::Controllers`; `ThreadPool` lives under `CXPM/Core/Threading/` but declares
   `namespace CXPM::Threading`. Their tests stay colocated with the header file itself and use
   the class's actual declared namespace, e.g. `CXPM::Controllers::Testing`, even though that
   diverges from the folder path.)
2. Each test file SHALL declare exactly one `TestGroup`-derived struct (from `CXPM::Testing`),
   named `<Subject>Test`, and instantiate one `inline static` instance of it so that constructing
   the translation unit's static objects self-registers the group into
...
4. Every `Testing/` subtree (the top-level harness and each colocated per-namespace one) SHALL be
   excluded from `cxpm-interface`'s installed `FILE_SET HEADERS`
   (`libraries/cxpm-interface/CMakeLists.txt`), so consumers of the installed package never see
   test-only headers.
5. CMake SHALL discover every `*Test.hpp` header located under any `Testing/` directory via
   `file(GLOB_RECURSE ... CONFIGURE_DEPENDS ...)` and generate one aggregate translation unit
   that `#include`s all of them, exactly as `tests/CMakeLists.txt` does — this surfaces ODR
   violations and name
