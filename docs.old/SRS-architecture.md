...
| **A13** | `ApplicationView` | `--uninstall`/`-u` is documented in `print_usage()` and has a stub private `uninstall()` method, but `run()` never checks for it — the flag is entirely non-functional | `ApplicationView.hpp:106`, `171-173` | Medium | Wire it up, or remove it from the help text until implemented |
...
| **A16** | `Core::{SharedPointer,UniquePointer,WeakPointer}`, `Utils/Print.hpp`, `ProgramOptions::OptionDescriptor` | Thin wrapper/utility headers never used anywhere in the codebase | grep-confirmed dead | Low | Start using them for a stated reason, or remove them to shrink the API surface a test suite has to cover |
...
| **A35** | `ApplicationView` | `OptionsDescriptorCollection` is constructed in `run()` and then never used — `print_usage()` is a hardcoded string instead of being generated from the schema | `ApplicationView.hpp:30-31` | Low | Either use it to generate the usage text, or remove the dead variable |
